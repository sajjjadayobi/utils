...
def price2persian(inputN):
    ...
    dahha = ["ده", "یازده", "دوازده", "سیزده", "چهارده",
             "پانزده", "شانزده", "هفده", "هجده", "نوزده"]
    tabist = yekan + dahha
    dahgan = ["", "ده", "بیست", "سی", "چهل",
    ...
    tahezarNs, tahezarNi = tahezar(
        int(empty.join(tahezarN))), int(empty.join(tahezarN))
    hezarNs, hezarNi = tahezar(
        int(empty.join(hezarN))), int(empty.join(hezarN))
    millionNs, millionNi = tahezar(
        int(empty.join(millionN))), int(empty.join(millionN))
    milliardNs, milliardNi = tahezar(
        int(empty.join(milliardN))), int(empty.join(milliardN))
    trillionNs, trillionNi = tahezar(
        int(empty.join(trillionN))), int(empty.join(trillionN))
    quadrillionNs, quadrillionNi = tahezar(
        int(empty.join(quadrillionN))), int(empty.join(quadrillionN))
    quintillionNs, quintillionNi = tahezar(
        int(empty.join(quintillionN))), int(empty.join(quintillionN))
    sextillionNs, sextillionNi = tahezar(
        int(empty.join(sextillionN))), int(empty.join(sextillionN))
    septillionNs, septillionNi = tahezar(
        int(empty.join(septillionN))), int(empty.join(septillionN))
    octillionNs, octillionNi = tahezar(
        int(empty.join(octillionN))), int(empty.join(octillionN))
    nonillionNs, nonillionNi = tahezar(
        int(empty.join(nonillionN))), int(empty.join(nonillionN))
    decillionNs, decillionNi = tahezar(
        int(empty.join(decillionN))), int(empty.join(decillionN))
    undecillionNs, undecillionNi = tahezar(
        int(empty.join(undecillionN))), int(empty.join(undecillionN))
    duodecillionNs, duodecillionNi = tahezar(
        int(empty.join(duodecillionN))), int(empty.join(duodecillionN))
    tredecillionNs, tredecillionNi = tahezar(
        int(empty.join(tredecillion))), int(empty.join(tredecillion))
    quattuordecillionNs, quattuordecillionNi = tahezar(
        int(empty.join(quattuordecillonN))), int(empty.join(quattuordecillonN))
    quindecillionNs, quindecillionNi = tahezar(
        int(empty.join(quindecillionN))), int(empty.join(quindecillionN))
    sexdecillionNs, sexdecillionNi = tahezar(
        int(empty.join(sexdecillionN))), int(empty.join(sexdecillionN))
    septendecillonNs, septendecillonNi = tahezar(
        int(empty.join(septendecillionN))), int(empty.join(septendecillionN))
    octodecillionNs, octodecillionNi = tahezar(
        int(empty.join(octodecillionN))), int(empty.join(octodecillionN))
    novemdNs, novemdNi = yekan[int(empty.join(novemdN))], int(
        empty.join(novemdN))
